...
- **Gongshu Workspace v0.5**：当前 Phone 主流程冻结在可信空间链条，输出同一 Scene Snapshot 关联的 Depth、Camera Intrinsics、Target Point Cloud 与 Target XYZ，并在 `SPATIAL_READY` 停止。仓库保留早期 v0.6 抓取 / MuJoCo 研究代码，但本轮不会自动调用或把它当作 v0.5 结果。
...
当前验证环境包括 PySide6 6.8.3、OpenCV 4.11、MuJoCo 3.9.0、robosuite 1.5.2、PyTorch 2.13.0 CPU、torchvision 0.28.0、Ultralytics 8.4.128、aiohttp 3.14.3、aiortc 1.15.0 和 cryptography 50.0.1。
...
进入 Gongshu 后不再显示独立 Camera Input 页面。默认 Pipeline State 为 `LIVE`，Live RGB 是主视图；点击辅助视图可进入 Manual Pin，选择 Auto Follow 后恢复阶段跟随。Phone Live RGB 连接后先点击 `分析目标 Analyze Targets`：本地服务冻结一帧、生成实例候选并在原图上显示掩膜；点击候选后才进入 `TARGET_SELECTED` 并启用空间链分析。v0.5 严格复用同一个 Scene Snapshot，依次进入 `SCENE_CAPTURED → SPATIAL_ANALYSIS → SPATIAL_READY`，不会自动进入 Grasp Planning。空间主视图按真实 backend stage 显示 Model Loading、Depth Estimating、Point Cloud Building、Spatial Computing 与 backend elapsed time；没有虚假百分比进度。普通 Phone RGB 使用可配置标称对角视场角构造 `NOMINAL_FOV / UNCALIBRATED` 投影参数；当前 metric-scaled 单目结果只标记 `Approx. Metric`。`SpatialObservation.geometry_diagnostics` 会在本地 API 中记录 Snapshot / Depth / Mask / Model Input 尺寸、Geometry Chain、`fx/fy/cx/cy`、来源与标定状态、目标 bbox、中位深度、有效深度比例、点云范围和质心 XYZ。
...
后续 v0.6 将按 `Grasp Detection → Top-K Candidates → Feasibility Filtering → Candidate Ranking → Best Executable Grasp` 重构。仓库中早期单候选规划代码仅保留为研究基线；`maximum_object_extent_m = 0.35` 与 Panda `0.01–0.08 m` 宽度保护没有放宽。

后续 v0.7 才正式接入动态 MuJoCo Success + Failure Visualization。本轮 v0.5 不生成或冒充 Grasp / MuJoCo 结果。扫码、证书、连接状态与高清 Capture 仍位于 `连接设置 Camera Setup`；原离线运行读取位于 `历史运行 History` 次级入口。
...
构建器检查运行依赖与模型完整性，把经过 SHA-256 校验的 FastSAM / Depth Anything V2 复制到 `models/`，写入 `release-models.json` 和第三方声明后生成 ZIP。`--prepared-app` 必须已经包含可运行 EXE 或源码启动器及 `frontend/`；该脚本不把模型写回 Git，也不把未冻结的源码目录误称为自包含 EXE。
...
- 真实 Camera v0.5 主链路目前可生成 RGB Frame、手动目标、单目近似深度、相机内参、目标点云与 Camera Frame XYZ；它不是标定后的 Camera→Robot / World 坐标闭环。
...
- Physical Phone v0.5 验收仍由用户完成；Phone Mode 不包含任何具体手机型号硬编码，自动化和静态样本结果不得描述为实体手机精度实测。
