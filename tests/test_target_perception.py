...
class TargetPerceptionServiceTests(unittest.TestCase):
    ...
    def test_unknown_target_selection_locks_same_frozen_frame(self) -> None:
        ...
        self.assertEqual(scene_snapshot.frame.frame_id, frame.frame_id)
        self.assertEqual(scene_snapshot.target.instance_id, target["id"])
        self.assertIsNot(scene_snapshot.frame.rgb, frame.rgb)
        self.assertFalse(scene_snapshot.frame.rgb.flags.writeable)
