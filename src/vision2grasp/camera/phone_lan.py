...
class PhoneLANProvider:
    ...
    async def _setup_page(self, request: web.Request) -> web.Response:
        ...
        html = f"""<!doctype html><html lang=\"zh-CN\"><head><meta charset=\"utf-8\"><meta name=\"viewport\" content=\"width=device-width,initial-scale=1\"><title>GONGSHU CAMERA 本地证书设置</title><style>body{{margin:0;background:#061a35;color:#e8f4ff;font:16px/1.7 system-ui,'Microsoft YaHei';}}main{{max-width:680px;margin:auto;padding:44px 24px;}}small{{color:#62d6bf;letter-spacing:.14em;}}h1{{font-size:32px;margin:10px 0;}}section{{margin-top:24px;padding:22px;border:1px solid #31557b;border-radius:16px;background:#0b2748;}}a{{display:inline-block;margin:12px 0;padding:13px 18px;border-radius:10px;background:#1e7af0;color:white;text-decoration:none;font-weight:700;}}code{{display:block;overflow-wrap:anywhere;color:#9fc9f5;font-size:11px;}}li{{margin:8px 0;}}</style></head><body><main><small>LOCAL TRUST SETUP</small><h1>GONGSHU CAMERA</h1><p>这是 Gongshu 的本地局域网证书。只需在本手机安装一次；证书私钥始终保留在电脑。</p><section><h2>首次设置</h2><ol><li>下载本地 CA 证书。</li><li>在系统设置中选择“从存储设备安装证书 / CA 证书”。</li><li>核对下方 SHA-256 指纹与电脑界面一致。</li><li>安装完成后，重新扫描电脑上的“连接手机”二维码。</li></ol><a href=\"/xuanshu-camera-ca.crt\" download>下载 Gongshu 本地 CA</a><p>SHA-256</p><code>{fingerprint}</code></section></main></body></html>"""
